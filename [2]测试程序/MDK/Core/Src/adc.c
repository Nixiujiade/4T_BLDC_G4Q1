...
#define bufSize   1
uint16_t buf2[bufSize]={0};
uint32_t Value2=0;
float endValue2=0;


float AdcToDmaGetValue2(void)
{
//	HAL_ADC_Start_DMA(&hadc2,(uint32_t*)buf2,bufSize);
//    //等待DMA搬运完成
//	while(__HAL_DMA_GET_FLAG(&hdma_adc2,DMA_FLAG_TC2))
//	{
...
//		Value2=buf2[i];
//	}
//	endValue2=(float)Value2*(3.3f/4096.0f);
	return endValue2;
}

uint16_t buf1[bufSize]={0};
uint32_t Value1=0;
float endValue1=0;


float AdcToDmaGetValue1(void)
{
//	HAL_ADC_Start_DMA(&hadc1,(uint32_t*)buf1,bufSize);
//    //等待DMA搬运完成
//	while(__HAL_DMA_GET_FLAG(&hdma_adc1,DMA_FLAG_TC1))
//	{
//        //hdma_adc1要进行外部声明，cbuemx生成时选分别生产.c .h文件即可在adc.c中找到
...
//		Value1=buf1[i];
//	}
//	endValue1=(float)Value1*(3.3f/4096.0f);
	return endValue1;
}
/* USER CODE END 1 */
